...
from daep.SpectraLayers import spectraTransceiverEncoder, spectraTransceiverMAEDecoder2stages
...
def train(epoch=1000, lr = 2.5e-4, bottlenecklen = 4, bottleneckdim = 4, 
    ...
    for ep in progress_bar:
        ...
        if (ep+1) % save_every == 0:
            ...
            target_save = f"../ckpt/AstroM3spectra_mae_{bottlenecklen}-{bottleneckdim}-{encoder_layers}-{decoder_layers}-{model_dim}_{encoder_heads}_{decoder_heads}_hiddenlen{hidden_len}_concat{concat}_corrattnonly{cross_attn_only}_lr{lr}_epoch{ep+1}_batch{batch}_mask{mask_rate}_aug{aug}.pth"
            torch.save(mymae, target_save)
            plt.plot(epoches, epoch_loss)
            plt.show()
            plt.savefig(f"./logs/AstroM3spectra_mae_{bottlenecklen}-{bottleneckdim}-{encoder_layers}-{decoder_layers}-{model_dim}_{encoder_heads}_{decoder_heads}_hiddenlen{hidden_len}_concat{concat}_corrattnonly{cross_attn_only}_lr{lr}_batch{batch}_mask{mask_rate}_aug{aug}.png")
            plt.close()
