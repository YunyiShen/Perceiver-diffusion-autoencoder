...
from daep.SpectraLayers import spectraTransceiverEncoder2stages, spectraTransceiverMAEDecoder2stages
...
def train(epoch=1000, lr = 2.5e-4, bottlenecklen = 4, bottleneckdim = 4, 
          ...
          encoder_layers = 4, 
          decoder_layers = 4, mask_rate = 0.30, 
    ...
    spectraDecoder = spectraTransceiverMAEDecoder2stages(
                    bottleneck_dim = bottleneckdim,
                    model_dim = model_dim,
                    ff_dim = model_dim,
    ...
    mymae = unimodalmae(spectraEncoder, spectraDecoder, mask_rate).to(device)
    ...
    for ep in progress_bar:
        ...
        if (ep+1) % save_every == 0:
            ...
            target_save = f"../ckpt/AstroM3spectra_mae2stages_{bottlenecklen}-{bottleneckdim}-{encoder_layers}-{decoder_layers}-{model_dim}_{encoder_heads}_{decoder_heads}_hiddenlen{hidden_len}_concat{concat}_corrattnonly{cross_attn_only}_lr{lr}_epoch{ep+1}_batch{batch}_mask{mask_rate}_aug{aug}.pth"
            torch.save(mymae, target_save)
            plt.plot(epoches, epoch_loss)
            plt.show()
            plt.savefig(f"./logs/AstroM3spectra_mae2stages_{bottlenecklen}-{bottleneckdim}-{encoder_layers}-{decoder_layers}-{model_dim}_{encoder_heads}_{decoder_heads}_hiddenlen{hidden_len}_concat{concat}_corrattnonly{cross_attn_only}_lr{lr}_batch{batch}_mask{mask_rate}_aug{aug}.png")
            plt.close()
