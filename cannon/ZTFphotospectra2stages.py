...
def train(epoch=1000, lr = 2.5e-4, bottlenecklen = 4, bottleneckdim = 4, 
    ...
    for ep in progress_bar:
        ...
        if (ep+1) % save_every == 0:
            ...
            target_save = f"../ckpt/ZTFphotospectra_daep2stages_{bottlenecklen}-{bottleneckdim}-{spectra_tokens}-{photometry_tokens}-{encoder_layers}-{decoder_layers}-{model_dim}_concat{concat}_mixerselfattn{mixer_selfattn}_lr{lr}_persampledrop{persample_dropping}_modaldropP{dropping_prob}_epoch{ep+1}_batch{batch}_reg{regularize}_aug{aug}.pth"
            torch.save(mydaep, target_save)
            plt.plot(epoches, epoch_loss)
            plt.show()
            plt.savefig(f"./logs/ZTFphotospectra_daep2stages_{bottlenecklen}-{bottleneckdim}-{spectra_tokens}-{photometry_tokens}-{encoder_layers}-{decoder_layers}-{model_dim}_concat{concat}_mixerselfattn{mixer_selfattn}_lr{lr}_persampledrop{persample_dropping}_modaldropP{dropping_prob}_batch{batch}_reg{regularize}_aug{aug}.png")
            plt.close()
