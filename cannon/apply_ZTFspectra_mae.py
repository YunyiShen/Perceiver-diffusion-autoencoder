...
ckpt = "ZTFspectra_mae_4-4-4-2-128_heads8-8_concatTrue_corrattnonlyFalse_lr0.00025_epoch2000_batch128_mask0.3_aug5"
...
axes = axes.flatten()
for i in range(20):
    axes[i].plot(x_ori['wavelength'][i][~x_ori['mask'][i]] ,
                 x_ori['flux'][i][~x_ori['mask'][i]] ,
                 color = "red",
                 label = "ground truth" if i == 0 else None
                 )
    axes[i].plot(recon['wavelength'][i][~recon['mask'][i]]* wavelength_std + wavelength_mean ,
                 recon['flux'][i][~recon['mask'][i]] ,
                 color = "blue",
...
plt.tight_layout()
fig.show()
fig.savefig("spectra_recon_mae.pdf")
plt.close()
