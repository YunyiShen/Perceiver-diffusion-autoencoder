...
which = "_test"
daep = dict(np.load( f"ZTFphotometric_daep_2-2-4-4-128_concatTrue_corrattnonlyFalse_lr0.00025_epoch2000_batch128_reg0.0_aug5{which}.npz").items())
mae = dict(np.load(f"ZTFphotometric_mae_2-2-4-2-128_concatTrue_corrattnonlyFalse_lr0.00025_epoch2000_batch128_mask0.75_aug5{which}.npz").items())
vae = dict(np.load(f"ZTF_photometry_vaesne_2-2-128-4_heads4_0.00025_epoch2000_batch128_aug5_beta0.1{which}.npz").items())
...
for seed in tqdm(seeds):
    ...
    metric_name_base = f"classification_metric/ZTFphotometry/ZTFphotometry_fewshot_{training_proportion}_{lr}_{epochs}_{hidden}_{seed}"
    ckpt_name_base = f"classification_ckpt/ZTFphotometry/ZTFphotometry_fewshot_{training_proportion}_{lr}_{epochs}_{hidden}_{seed}"
